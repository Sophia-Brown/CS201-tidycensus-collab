...
# We chose the data about educationnal attainment of Bachelor's degree

edu_attainment <- get_acs(
  geography = "tract",
  variables = c(percent_poverty = "DP02_0065P"),
  state = "HI",
  county = "Honolulu",
  geometry = TRUE,
  output = "wide",
...
```{r, message=FALSE, warning=FALSE, results='hide'}
edu_attainment_map <- ggplot(edu_attainment) +
  geom_sf_interactive(
    aes(fill = percent_povertyE,
        tooltip = paste0(NAME, ": ", percent_povertyE, "%"),
        data_id = NAME),
    color = NA
  ) +
  scale_fill_viridis_c(option = "plasma", name = "Poverty (%)") +
  theme_void() +
  labs(title = "Percentage of adults aged 25 and older in Honolulu who hold a bachelor's degree")

girafe(ggobj = edu_attainment_map, width = 7, height = 4)
```
...
years <- 2010:2018 # specify years
names(years) <- years

edu_attainment_2019_2023 <- map_dfr(years, ~{
  get_acs(
    geography = "county",
    variables = c(percent_BA = "DP02_0065P"),
    state = "HI",
    county = "Honolulu",
    year = .x,
    survey = "acs5")}, .id = "year")
...
ggplot(edu_attainment_2019_2023, aes(x = year, y = estimate, group = 1)) + 
  geom_line() + 
  geom_point() +
  labs(
    title = "Honolulu County Educational Attainment (ACS Rolling 5-year Estimates)",
    x = "ACS Release Year (5-year window)",
...
# more informative and nicer to look at
...
ggplot(edu_attainment_2019_2023, aes(x = year, y = estimate, group = 1)) + 
  ...
  geom_point(color = "steelblue", size = 2) + 
  theme_minimal() + 
  labs(title = "Honolulu County Educational Attainment (ACS Rolling 5-year Estimates)",
    x = "ACS Release Year (5-year window)",
    y = "Percent with Bachelor's Degree or Higher",
       caption = "Shaded area represents margin of error around the estimate")
...
oahu_family_ba_income <- get_acs(
  geography = "tract",
  variables = c(percent_BA = "DP02_0065P", # Percent with BA
                percent_icome = "DP03_0062E"), # household income 
  state = "HI",
  county = "Honolulu",
  geometry = TRUE,
  output = "wide",
...
ggplot(oahu_family_ba_income, aes(x = percent_BAE,y = percent_icome)) +
  geom_point() +
  geom_smooth() + # adds a regression line (LOESS)
  stat_cor() + # prints R and p
  theme_minimal() + 
  labs(title = "Comparing Rates of Higher Education to Median Household Income, Honolulu County",
       subtitle = "ACS 5-year, 2023 release",
       x = "Percent with BA ",
       y = "House Hold Income",
       caption = "Shaded area represents margin of error around the estimate")
...
In the mapping, it shows that the Western O'ahu tends to have the lower rate. In contrast, the areas like waikiki, hawaii kai, and manoa seems to have high rate.

In the time series, it shows the percentage of adults aged 25 and older in Honolulu who hold a bachelor's degree. When I tried to plotted from 2014 - 2023 similar to example, the data show a sudden spike between 2018 and 2019. So, we changed a range between 2010 to 2018.

From 2010 to 2018, the percentage gradually increased. So, younger generation can have the higher education levels.

In the scatter plot part, we compare the Percent with Bachelor's degree and household income. The Correlation is R = 0.42 so it kind of like a positive relationship. So, it means percent with BA increase , income also increase. And p-value is 1.3e-14, so it means statistically significant, not a random chance. And x-axis ended near 50%, so it can represent that Honolulu's most educated areas, about half of adults hold a bachelor's degree.
